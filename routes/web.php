...
use App\Http\Controllers\Web\VendorPortalController;
...
Route::prefix('ui')->name('ui.')->middleware(['auth', 'program.enabled'])->group(function () {
    ...
    Route::middleware('role:' . UserRole::SUPER_ADMIN->value . ',' . UserRole::OWNER->value . ',' . UserRole::FINANCE->value)->group(function () {
        Route::get('/profit-loss', [ProcurementUiController::class, 'profitLossReport'])->name('profit-loss.index');
        Route::get('/profit-loss/export', [ProcurementUiController::class, 'exportProfitLossExcel'])->name('profit-loss.export');
        Route::get('/profit-loss/export-pdf', [ProcurementUiController::class, 'downloadProfitLossPdf'])->name('profit-loss.export-pdf');
    });

    Route::middleware('role:' . UserRole::VENDOR_ADMIN->value)->group(function () {
        Route::get('/vendor/portal', [VendorPortalController::class, 'dashboard'])->name('vendor.portal');
        Route::post('/vendor/stock/{product}', [VendorPortalController::class, 'updateStock'])->name('vendor.stock.update');
        Route::post('/vendor/margin-payments', [VendorPortalController::class, 'storeMarginPayment'])->name('vendor.margin-payments.store');
        Route::post('/vendor/margin-payments/{payment}/upload-proof', [VendorPortalController::class, 'uploadProof'])->name('vendor.margin-payments.upload-proof');
    });

    Route::middleware('role:' . UserRole::SUPER_ADMIN->value . ',' . UserRole::OWNER->value . ',' . UserRole::FINANCE->value)->group(function () {
        Route::get('/finance/vendor-margin-payments', [VendorPortalController::class, 'financeIndex'])->name('finance.vendor-margin-payments.index');
        Route::post('/finance/vendor-margin-payments/{payment}/approve', [VendorPortalController::class, 'approveMarginPayment'])->name('finance.vendor-margin-payments.approve');
        Route::post('/finance/vendor-margin-payments/{payment}/reject', [VendorPortalController::class, 'rejectMarginPayment'])->name('finance.vendor-margin-payments.reject');
    });
    ...
    Route::get('/notifications/{notification}/open', [ProcurementUiController::class, 'openNotification'])->name('notifications.open');
